...
if __name__ == "__main__":
    ...
    test(model, tokenizer, "", "テンジョウノケンナオコガキニナッテネムレナイ")
    test(model, tokenizer, "", "ナノナイホシガソラニオチタラ")
    test(model, tokenizer, "", "キミトマタアエタラ")
    test(model, tokenizer, "", "モハヤヤマダダケデキョクツクレルカラ")
    test(model, tokenizer, "降るほうの", "アメ")
    test(model, tokenizer, "舐める方の", "アメ")
