...
LEARNING_RATE = 1e-4
EPOCHS = 5
DATASET_PATH = "../dataset/train_data.bin"
TOKENIZER_PATH = "../model/tokenizer/tokenizer.json"
SAVE_PATH = "./model/weights/mocho.pth"

# --- データセット定義 ---
class BinaryDataset(IterableDataset):
    def __init__(self, bin_path, seq_len, tokenizer_path):
        # メモリマップとして読み込み（メモリを消費せず、必要な時にディスクから読む）
    ...
    def __iter__(self):
        # 学習時はランダムな位置から SEQ_LEN+1 切り出す
        # 本来は文の区切りを考慮すべきですが、まずは速度重視のバルク読み込み
        data_len = len(self.data)
        while True:
            # ランダムな開始位置を選択
            idx = np.random.randint(0, data_len - self.seq_len - 1)
            chunk = self.data[idx : idx + self.seq_len + 1].astype(np.int64)
            
            x_ids = chunk[:-1]
            y_ids = chunk[1:]
            
            # マスク作成（output_id 以降を 1.0 にする）
            # ここを効率化するために numpy で一括処理
            mask = np.zeros(self.seq_len, dtype=np.float32)
            try:
                # チャンク内に output_id があるか
                out_pos = np.where(x_ids == self.output_id)[0][0]
                mask[out_pos:] = 1.0
            except IndexError:
                # output_id がない場合はこのサンプルを飛ばすか、全部 0 にする
                pass

            yield torch.from_numpy(x_ids), torch.from_numpy(y_ids), torch.from_numpy(mask)

# --- 準備 ---
tokenizer = Tokenizer.from_file(TOKENIZER_PATH)
dataset = BinaryDataset(DATASET_PATH, tokenizer, SEQ_LEN)
