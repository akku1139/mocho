...
BIN_PATH_MOZC = "../dataset/train_data_mozc.bin"
IDX_PATH_MOZC = "../dataset/train_indices_mozc.bin"
TOKENIZER_PATH = "../model/tokenizer/tokenizer.json"

# 保存パスの分離
MODEL_SAVE_PATH = "../model/weights/v1.3/mocho.safetensors"
OPT_SAVE_PATH = "../model/weights/v1.3/optimizer_state_mixed.pth"
...
def main():
    logger('train with mixed dataset')
    dataset_wikipedia = BinaryDataset(BIN_PATH_WIKIPEDIA, IDX_PATH_WIKIPEDIA, SEQ_LEN, TOKENIZER_PATH)
    dataset_mozc = BinaryDataset(BIN_PATH_MOZC, IDX_PATH_MOZC, SEQ_LEN, TOKENIZER_PATH)

    # 2. 比率の計算
    # ターゲット比率を 10% とする場合：
    # dataset_wikipedia_len : (dataset_mozc_len * repeat) = 90 : 10
    # repeat = (dataset_wikipedia_len * 10) / (dataset_mozc_len * 90)
    ...
    logger(f"Wiki samples: {dataset_wikipedia_len}, Mozc samples: {dataset_mozc_len}")
    logger(f"Repeating Mozc {repeat_count} times to achieve ~{target_ratio*100}% ratio")

    # 3. Mozcをリピートさせたリストを作成して結合
    # リストの掛け算 [obj] * n は参照コピーなのでメモリを食いません
    combined_dataset = ConcatDataset([dataset_wikipedia] + [dataset_mozc] * repeat_count)

    # 4. あとはDataLoaderに渡すだけ（shuffle=Trueにより、WikiとMozcが毎ステップ混ざる）
    loader = DataLoader(
        combined_dataset,
    ...
    # これはどっちからとってもいい
    output_token_id = dataset_wikipedia.output_id
